...
if __name__ == "__main__":
    ...
    html_completer = WordCompleter(
        ["lcd", "lls", "upload", "download", "exit", "pwd", "cat", "clear"]
    )
        ...
        if len(read_in_user_input.split(" ")) == 1:
            ...
            print(
                subprocess.check_output(
                    ["ls", "-la", read_in_user_input.split(" ")[1]]
                ).decode("utf-8")
            )
    ...
    try:
        if (
            ...
            logging.error(
                "Wrong number of args, require {username, target, port, password|key}"
            )
            sys.exit(2)
        else:
            while True:
                transfer = SFTPTransfer(
                    args.target, args.port, args.username, args.password, args.key
                )

                session = PromptSession()
                read_in_user_input = session.prompt(
