...
def pastable():
    ...
    cprint("python3 client.py -i {} -p {} -f index.html -b decrypted\n".format(args.ipaddress, args.port), "light_blue", attrs=['bold'])


def do_encrypt(in_file, outfile):
    os.system('cp {} {}'.format(in_file, outfile))
    do_enc = os.system('openssl enc -aes-256-cbc -pbkdf2 -salt -pass pass:{} -in {} -out {}'.format(password_generation(), in_file, outfile))
    return do_enc

def password_generation(size=20, char=string.ascii_lowercase + string.ascii_uppercase + string.digits):
