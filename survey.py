...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c",
        "--config",
        help="path to config file with commands to run on the remote host seperated by a new line.",
        action="store",
        dest="config",
    )
    parser.add_argument(
    ...
    try:
        ...
        if (
            ...
            logging.error(
                "Wrong number of args, require {username, target, password|key, port, config}"
            )
            sys.exit(2)
        # this is if user sets a log file, if they do we will log commands to specified logfile
        elif args.logfile:
            target_session = REMOTECONNECT(
                args.target, args.username, args.password, args.key, args.port
            )
