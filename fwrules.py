...
def lo_set_default_rules():
    ...
    in_rules = [
        ipt
        + " -t filter -A INPUT -i eth0 -s 0.0.0.0/0 -d "
        + my_ip
        + " -m state --state ESTABLISHED -j ACCEPT",
        ipt
        + " -t filter -A INPUT -i eth0 -p icmp -s 0.0.0.0/0 -d "
        + my_ip
        + " -m state --state ESTABLISHED,RELATED -j ACCEPT",
    ]
    out_rules = [
        ipt
        + "-t filter -A OUTPUT -o eth0 -p tcp -s "
        + my_ip
        + " -d 0.0.0.0/0 --dport 80 -j ACCEPT -m state --state NEW,ESTABLISHED",
        ipt
        + "-t filter -A OUTPUT -o eth0 -p tcp -s "
        + my_ip
        + " -d 0.0.0.0/0 --dport 443 -j ACCEPT -m state --state NEW,ESTABLISHED",
        ipt
        + "-t filter -A OUTPUT -o eth0 -p udp -s "
        + my_ip
        + " -d 0.0.0.0/0 --dport 53 -j ACCEPT",
        ipt
        + "-t filter -A OUTPUT -o eth0 -p icmp -s "
        + my_ip
