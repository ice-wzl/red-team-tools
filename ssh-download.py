...
def do_download(socket, username, structure):
    ...
    subprocess.Popen(
        [
            ...
            "-e",
            "ssh -S {} {}@ 'cat {}' > {}/{}".format(
                socket, username, r_path, structure, file_name
            ),
        ]
    )
    return 0
...
def download_dir(socket, username, structure):
    ...
    files_in_dir = subprocess.Popen(
        [
            "xterm",
            "-e",
            'ssh -S {} {}@ "ls {}" > /tmp/temp'.format(socket, username, r_path),
        ],
        ...
        for i in read_in.split():
            subprocess.Popen(
                [
                    ...
                    "-e",
                    "ssh -S {} {}@ 'cat {}/{}' > {}/{}".format(
                        socket, username, r_path, i, l_path, i
                    ),
                ]
            )
...
def do_upload(socket, username):
    ...
    subprocess.Popen(
        [
            "xterm",
            "-e",
            "ssh -S {} {}@ 'cat > {}' < {}".format(socket, username, r_path, l_path),
        ]
    )
    return 0
...
def do_upload_large(socket, username):
    ...
    subprocess.Popen(
        [
            "xterm",
            "-e",
            "ssh -S {} {}@ 'gzip > {}' < {}".format(socket, username, r_path, l_path),
        ]
    )
    return 0
...
def spawn_shell(socket, username):
    ...
    if shell_type == "":
        shell_type = "/bin/sh"
    subprocess.Popen(
        ["xterm", "-e", 'ssh -S {} {}@ "{}"'.format(socket, username, shell_type)],
        start_new_session=True,
    )
    return 0
...
def single_cmd(socket, username):
    ...
    cmd = input("Enter cmd: ")
    subprocess.Popen(
        ["xterm", "-e", 'ssh -S {} {}@ "{}"'.format(socket, username, cmd)],
        start_new_session=True,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        dest="socket",
        help="The socket file to connect to",
    )
    parser.add_argument(
        "-u",
        "--username",
    ...
    try:
        while keep_going:
            ...
            options = session.prompt(
                message=message, style=style, completer=html_completer
            )
